    ...
    <div id="project-list" className="sm:columns-2 lg:columns-3 p-2 gap-4 pb-8 ">
      {projects.map((project) => (
        <div
          ...
          className={`project pb-4 ${styles.projectItem} ${selectedProjectId === project.id ? `${styles.scaled} ${styles.layerShadowDark}` : ''} ${isHidden && selectedProjectId === project.id ? styles.hidden : ''}`}
          onClick={() => handleClick(project.id)}
          style={{ breakInside: 'avoid' }}
        >
          <div className="w-full text-zinc-950 tracking-wide rounded-xl">
            <div
              className={`${styles.projectItem} w-full flex-col relative flex group bg-white rounded-xl layer-shadow layer-shadow-hover sm:hover:scale-[1.01] duration-300 ease-[cubic-bezier(0,0,.5,1)]`}
              style={
            ...
            <div
              style={selectedProjectId === project.id ? { opacity: '0', transition: 'opacity 0.1s' } : {}}
